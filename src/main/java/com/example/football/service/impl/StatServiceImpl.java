...
import com.example.football.models.dto.rootDto.StatRootSeedDto;
...
public class StatServiceImpl implements StatService {
    ...
    public String importStats() throws JAXBException, FileNotFoundException {
        ...
        xmlParser
                .fromFile(STATS_FILE_PATH, StatRootSeedDto.class)
                .getStats()
                .stream()
                .filter(statSeedDto -> {
                    boolean isValid = validationUtil.isValid(statSeedDto)
                            && !statRepository.findByEnduranceAndPassingAndShooting(Float.parseFloat(statSeedDto.getEndurance().toString()),
                            Float.parseFloat(statSeedDto.getPassing().toString()),
                            Float.parseFloat(statSeedDto.getShooting().toString())).isPresent();
                    sb
                            .append(isValid ? String.format("Successfully import Stat %.2f - %.2f - %.2f",
